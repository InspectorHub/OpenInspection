...
- **CSS**: Tailwind utilities + canonical v3 tokens defined in `src/styles/input.css`. No `font-black` outside stat numbers, no `rounded-2xl`, no `tracking-tightest`. The design system reference is at `[redacted]`.
