    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-50 dark:bg-slate-900">
        <div className="text-center p-8">
          <h1 className="text-2xl font-bold text-slate-900 dark:text-slate-100">Agreement Not Found</h1>
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-50 dark:bg-slate-900">
        <div className="text-center p-8 max-w-md">
          <h1 className="text-xl font-bold text-slate-900 dark:text-slate-100">Thank you</h1>
          <p className="text-slate-500 dark:text-slate-400 mt-2">
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <div className="bg-white dark:bg-slate-800 rounded-lg shadow-md overflow-hidden">
          ...
          <div className="px-6 py-6 sm:px-10 sm:py-8 border-b border-slate-100 dark:border-slate-700">
            <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-indigo-600 dark:text-indigo-400 mb-2">Document for Signature</p>
            <h1 className="text-xl font-bold text-slate-900 dark:text-slate-100 tracking-tight">{agreement.title}</h1>
          ...
          {agreement.signedAt || signed ? (
            <div className="px-6 py-8 sm:px-10 sm:py-10 text-center">
              ...
              <button
                ...
                className="inline-flex items-center gap-2 px-5 py-2.5 rounded-md bg-indigo-600 text-white text-sm font-bold hover:bg-indigo-700 transition-all"
              >
            ...
            <div className="px-6 py-6 sm:px-10 sm:py-8">
              <p className="text-sm font-bold text-slate-500 dark:text-slate-400 mb-4">Draw your signature below:</p>

              {/* Signature canvas */}
              <div className="border-2 border-slate-200 dark:border-slate-600 rounded-2xl overflow-hidden bg-slate-50 dark:bg-slate-800 mb-4" style={{ touchAction: "none" }}>
                <canvas
                  ref={canvasRef}
                  width={580}
                  height={180}
              ...
              {/* Error message */}
              {errorMsg && (
                <div className="mb-4 px-3 py-2 rounded-md bg-rose-50 dark:bg-rose-900/20 text-[13px] font-medium text-rose-700 dark:text-rose-300 text-center">
                  {errorMsg}
                </div>
              )}
              ...
              <div className="flex gap-3 mb-6">
                <button
                  onClick={clearSig}
                  className="flex-1 h-10 px-4 rounded-md border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-300 text-sm font-semibold hover:bg-slate-50 dark:hover:bg-slate-700 transition-all"
                >
                  Clear
                </button>
                <button
                  ...
                  disabled={submitting}
                  className="flex-[2] h-10 px-4 bg-indigo-600 text-white rounded-md font-bold text-sm hover:bg-indigo-700 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                >
              ...
              <div className="border-t border-slate-100 dark:border-slate-700 pt-4">
                ...
                {showDecline && (
                  <div className="mt-3 p-4 bg-rose-50 dark:bg-rose-900/10 rounded-lg border border-rose-100 dark:border-rose-800">
                    <label className="block text-[10px] font-bold text-rose-700 dark:text-rose-300 uppercase tracking-widest mb-2">Reason (optional)</label>
                    <textarea
                      value={declineReason}
                      onChange={(e) => setDeclineReason(e.target.value)}
                      rows={3}
                      className="w-full px-3 py-2 rounded-lg border border-rose-200 dark:border-rose-700 bg-white dark:bg-slate-800 text-sm text-slate-900 dark:text-slate-100 focus:ring-2 focus:ring-rose-500/20 outline-none"
