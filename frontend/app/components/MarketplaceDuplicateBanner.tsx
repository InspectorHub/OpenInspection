...
export function MarketplaceDuplicateBanner({ groups, onCompare, onUseNew, onKeepBoth }: MarketplaceDuplicateBannerProps) {
  ...
  return (
    <div className="rounded-lg border border-amber-200 bg-amber-50 dark:bg-amber-900/20 dark:border-amber-700 p-4" role="status">
      {groups.map((g) => (
        <div key={g.marketplaceId} className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="mt-2 flex items-center gap-2 flex-wrap">
              <button type="button" onClick={() => onCompare?.(g)} className="h-7 px-3 rounded-md bg-white dark:bg-slate-800 border border-amber-300 dark:border-amber-600 text-amber-700 dark:text-amber-300 text-[12px] font-bold hover:bg-amber-100 dark:hover:bg-amber-900/40 transition-colors focus:outline-none focus:ring-2 focus:ring-amber-500/30">
                Compare versions
              </button>
              ...
              <button type="button" onClick={() => { onKeepBoth?.(g); setDismissed(true); }} className="h-7 px-3 rounded-md text-amber-700 dark:text-amber-300 text-[12px] font-medium hover:bg-amber-100 dark:hover:bg-amber-900/40 transition-colors focus:outline-none focus:ring-2 focus:ring-amber-500/20">
                Keep both
              </button>
            </div>
          </div>
