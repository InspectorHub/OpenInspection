    ...
    <div className="px-6 py-6 max-w-5xl" data-testid="property-info-form">
      <header className="mb-6">
        <div className="flex items-center gap-2 mb-1">
          ...
          {filled === metaFields.length && (
            <span className="inline-flex items-center px-1.5 py-0.5 rounded-full text-[10px] font-bold bg-emerald-50 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400 ring-1 ring-inset ring-emerald-200 dark:ring-emerald-700">
              Complete
            </span>
          )}
        </div>
      ...
      {groups.map((g) => (
        <fieldset key={g} className="mb-6">
          <legend className="text-[11px] font-bold uppercase tracking-[0.15em] text-slate-400 dark:text-slate-500 mb-2">{g}</legend>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
            {fieldsByGroup(g).map((f) => (
              <label key={f.id} className="block">
                <span className="flex items-center gap-1 text-[10px] font-bold uppercase tracking-[0.18em] text-slate-500 dark:text-slate-400">
                  <span>{f.label}</span>
                  {inspection[`_prefilled_${f.id}`] && (
                    <span className="text-[9px] font-semibold text-indigo-500 dark:text-indigo-400 normal-case tracking-normal">Prefilled</span>
                  )}
                </span>
