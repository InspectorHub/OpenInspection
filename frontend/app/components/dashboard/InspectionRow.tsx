...
export function InspectionRow({ inspection: i, visibleColumns }: InspectionRowProps) {
  ...
  return (
    <div className="px-5 py-[13px] border-t border-slate-100 dark:border-slate-700 flex items-center gap-3 flex-wrap sm:flex-nowrap" data-test="inspection-row">
      ...
      >
        <p className="font-bold text-slate-900 dark:text-slate-100 truncate text-[14px]">
        ...
        <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-0.5">
          ...
          {i.siblingCount != null && i.siblingCount > 1 && (
            <span> · <span className="inline-flex items-center px-1.5 py-0.5 rounded bg-indigo-50 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-300 text-[10px] font-bold ring-1 ring-inset ring-indigo-200 dark:ring-indigo-800">{i.siblingCount} inspections</span></span>
          )}
        ...
        {isVisible(cols, "defectChips") && hasDefects && (
          <div className="mt-1 flex items-center gap-1.5">
            {i.defectStats!.safety > 0 && <span className="inline-flex items-center gap-1 px-1.5 py-0.5 text-[10px] font-bold rounded bg-rose-50 dark:bg-rose-900/30 text-rose-700 dark:text-rose-300">{i.defectStats!.safety} safety</span>}
            {i.defectStats!.recommendation > 0 && <span className="inline-flex items-center gap-1 px-1.5 py-0.5 text-[10px] font-bold rounded bg-amber-50 dark:bg-amber-900/30 text-amber-700 dark:text-amber-300">{i.defectStats!.recommendation} rec</span>}
      ...
      {isVisible(cols, "price") && i.price != null && i.price > 0 && (
        <div className="text-[12px] font-mono font-semibold text-slate-700 dark:text-slate-300 tabular-nums">
