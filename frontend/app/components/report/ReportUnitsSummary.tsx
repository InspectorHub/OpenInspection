      ...
      <div className="rounded-md border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <ul className="space-y-3 text-sm">
          {buildings.map((building) => (
            <li key={building.id}>
              <a href={`#unit-${building.id}`} className="font-bold text-slate-900 hover:text-indigo-600">{building.name}</a>
              <ul className="ml-3 mt-1 space-y-1">
                {childrenOf(units, building.id).map((floor) => (
                  <li key={floor.id}>
                    <a href={`#unit-${floor.id}`} className="font-medium text-slate-700 hover:text-indigo-600">{floor.name}</a>
                    <ul className="ml-3 mt-1 space-y-1">
                      {childrenOf(units, floor.id).map((unit) => {
                        const count = counts[unit.id] ?? 0;
                        return (
                          <li key={unit.id} className="flex items-center gap-2">
                            <a href={`#unit-${unit.id}`} className="text-slate-600 hover:text-indigo-600">{unit.name}</a>
                            {count > 0 && (
                              <span className="inline-flex items-center px-2 py-0.5 rounded-full bg-rose-50 text-rose-700 text-[10px] font-bold">
