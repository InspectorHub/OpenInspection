...
export function TemplateDriftBanner({ show, message, onUpgrade, onDismiss }: TemplateDriftBannerProps) {
  if (!show) return null;
  return (
    <div className="bg-amber-50 dark:bg-amber-900/20 border-l-4 border-amber-500 p-4 mb-4 rounded-r-lg">
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="text-sm font-bold text-amber-900 dark:text-amber-200">Template was updated</p>
          {message && <p className="text-xs text-amber-800 dark:text-amber-300 mt-1">{message}</p>}
        </div>
