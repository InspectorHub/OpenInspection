...
--                              signature. Mirrors `synthesizeDefaultSigner`,
--                              except that function copies `status` WITHOUT the
--                              signature, which is why it cannot be relied on
--                              to do this.
--   more than one signer row, none carrying a signature -> NOT handled, on
--                              purpose. No normal path produces it: signing
--                              writes the signer's own row, `findOrCreate` only
--                              merges signers into NON-terminal envelopes, and
--                              the self-heal creates exactly one row. The one
--                              way to reach it is a retention sweep that
--                              destroyed the signer signatures and did not
--                              reach the envelope — and there the correct
--                              action is to finish the sweep, never to backfill
--                              a signature into rows that were just anonymized.
--
-- THE GUARD BELOW IS THE POINT. Getting this wrong destroys retained legal
-- evidence, so the drop does not run on trust: a CHECK-constrained scratch
-- table takes the count of envelopes whose signature did NOT make it onto a
-- signer row. Zero passes; anything else fails the CHECK, the statement errors,
-- and the migration aborts BEFORE the DROP COLUMN. A deployment holding the
-- unhandled shape gets a failed migration and its data, not a silent loss.
--
-- Idempotent: the ids are derived from the request id rather than random, and
-- every write is guarded on the value being absent, so a re-run after a failed
-- attempt is a no-op. The `backfill:` id prefix also makes the synthesized rows
-- identifiable forever, which matters for evidence provenance.
