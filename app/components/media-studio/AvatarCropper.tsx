    ...
    <div className="fixed inset-0 z-[80] bg-[rgba(15,23,42,0.7)] flex flex-col" role="dialog" aria-modal="true" aria-label={m.media_avatar_crop_aria()}>
      ...
      <div className="bg-ih-bg-card border-t border-ih-border px-5 py-3 space-y-3">
        ...
        <div className="flex items-center justify-end gap-3">
          ...
          <button type="button" onClick={handleSave} disabled={busy || !pixels} className="h-9 px-4 rounded-md bg-ih-primary text-ih-fg-inverse text-[13px] font-bold hover:bg-ih-primary-600 disabled:opacity-50">{busy ? m.common_saving() : m.media_avatar_save()}</button>
        </div>
      </div>
    </div>
  );
