...
<!--
COUNSEL-READY v5 — rounds 29, 30, 31 and 32 landed. NOT APPROVED FOR PUBLICATION.

THIS FILE IS A DRAFT DERIVED FROM RULINGS. IT IS NOT A RULING SOURCE.
Counsel round 32 asked for this explicitly. The binding text is the archived Round
...
those disagree, this file is the one that is wrong:
  ...
  docs/legal/counsel-ruling-register.md          (superproject)
  docs/legal/2026-08-18-counsel-round-32-response.md
...
`npm run agent-terms:publish`. It prints the round 32 publish-gate checklist and
refuses while any line is red.

§17 IS GONE. Round 30 ruled that governing law may be omitted rather than named:
...
law, no venue and no arbitration, a dispute still has to find a forum. Counsel also
...
round 30 keeps this P0) and the retention model (round 13 approved no window). The
...
Round 32 found one that had drifted, which is the argument for keeping them.
-->


**Status:** counsel-ready draft (v5) — not published
...
*(Round 24c asked for an explicit no-agency statement and no draft had one. §1's
...
Kept SHORT on purpose — the authoritative list is the register's round 29/31/32
...
BLOCKING (both must be green before this publishes; the gate enforces both):
  ...
  · the retention model — round 13 approved no window, so §15 may not imply one

FOR THE OPERATOR TO DECIDE, not counsel:
  · the §14 liability floor — DECIDED: US$200, the most conservative of the values
    on the table. Counsel leaned US$100 and was explicit that no figure is legally
...
ENGINEERING, from round 32 and not a drafting item:
