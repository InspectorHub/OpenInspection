...
test.describe.serial('Agent unified link (Spec 3 Task 8)', () => {
  test.beforeAll(async ({ request }) => {
    ...
    const signupRes = await apiPost(request, '/api/agent-signup', '', {
      ...
      name: REGISTERED_AGENT.name,
      // Required: an agent is a third party and the account is
      ...
      // signup by design rather than by accident.
      termsAccepted: true,
    });
  ...
  test('Scenario 2 — unregistered agent: report link renders + signup CTA converts -> welcome on /agent-dashboard', async ({
    ...
    try {
      ...
      // third party and the account is not created without a recorded
      // acceptance. Asserting the body is visible first is the point: the tick
