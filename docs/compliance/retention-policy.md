...
## 2026-08-19.2 — the two pending tables close, and round 33 is partly withdrawn

**What changed.** `notifications` and `qbo_sync_errors` were the last two rules with
no decision. Both now have one, and round 34 withdrew part of round 33.

| Rule | Now | Why |
|---|---|---|
| `notifications` | **24 months from `created_at`** | The inbox, not the record that a communication happened — `automation_logs` answers that and is kept by design. Counsel explicitly declined anchoring on `read_at`/`archived_at`: an unread notice would become immortal, which turns a UI-state field into a retention control |
...
quote one. Counsel refused to give it a longer window for possibly explaining a
...
**Round 33's instruction on the legal-version tables is WITHDRAWN.** Both stay in
the sweep with reference-preserving executors. Counsel drew the distinction we had
