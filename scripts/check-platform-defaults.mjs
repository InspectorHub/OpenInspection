 ...
 *
 * `compliance/platform-defaults.jsonc` records counsel's round-20 B5 fields for
 ...
 *  4. `decision_type` is on counsel's taxonomy, and anything not yet classified
 ...
 *     `controller_override: required`. Counsel was explicit that how long client
...
/** Counsel's three values, plus the honest fourth. `unreviewed` is counted. */
...
function fieldProblems(entry) {
    ...
    if (!DECISION_TYPES.includes(entry.decision_type)) {
        problems.push(`${id}: decision_type '${entry.decision_type}' is not on counsel's taxonomy (${DECISION_TYPES.join(' · ')})`);
    }
    ...
    if (RETENTION_SHAPED.test(id) && (entry.decision_type !== 'controller_choice' || entry.controller_override !== 'required')) {
        problems.push(`${id}: retention-shaped defaults are controller_choice with controller_override 'required' (round 20 B5) — this says '${entry.decision_type}' / '${entry.controller_override}'`);
    }
    return problems;
}
