 ...
 * set is the eight categories counsel named, and it is closed. That makes
...
/**
 * The eight categories from counsel round 8, held here independently of the
 * source so the two can be compared. Order is counsel's, not alphabetical.
 */
...
for (const c of COUNSEL_CATEGORIES) {
    if (!declaredSet.has(c)) {
        errors.push(
            `NON_TRANSLATABLE_CATEGORIES is missing '${c}', which counsel round 8 named. ` +
...
for (const c of declaredCategories) {
    if (!counselSet.has(c)) {
        errors.push(
            ...
            `counsel categories this gate enforces. Widening the registry is a decision that ` +
...
for (const c of COUNSEL_CATEGORIES) {
    if (!covered.has(c)) {
        errors.push(
            `category '${c}' has NO manifest entry. Counsel named eight; a register carrying ` +
...
console.log(
    ...
    `${covered.size}/${COUNSEL_CATEGORIES.length} counsel categories, ` +
