 ...
 * legal determinations, sourced to a counsel ruling. One row carrying both is
 * exactly the confusion round 27 removed from the action and category labels;
 ...
 * `review_status` distinguishes a counsel ruling from an engineering reading
 ...
 * ARE a reading of the law, sourced to a counsel ruling.
 *  - `basis_kind` — `statutory_exclusion` (the definition excludes what we hold;
 *    round 26, CA-10) · `architecture_dependent` (only our own code keeps us out
...
export interface StatutoryClassification {
    ...
    /** CA-11 vocabulary: the only conditional axis counsel has ruled on so far. */
...
export const STATUTORY_CLASSIFICATIONS: StatutoryClassification[] = [
    ...
    // CA-11: counsel refused the one-word form because it hides the half of the
    ...
    // not hypothetical: this column is already TYPED for a device coordinate.
    {
        ...
        open_question: 'Counsel has not ruled on this column. Two facts to put to them: the strip-on-ingest re-encode fails OPEN when the images binding is absent, so a stored object can retain camera metadata the column does not; and takenAt is supplied by the client rather than read server-side',
