...
export const NON_TRANSLATABLE_OUT_OF_SCOPE: NonTranslatableOutOfScopeEntry[] = [
    // ── The two platform notices. The boundary this register mainly exists for.
    {
        ...
        reason: 'The Art. 13 report-view notice. A platform notice ABOUT the report, not a term OF it: it tells a recipient that a view counter exists, what it deliberately does not record, and how to object. It is already rendered through the message catalogue with a live es-419 value, deliberately, and that is correct — a transparency notice does its job only if the reader can read it. Freezing it to English would weaken the lawful basis it supports, which is the opposite of what the instrument rule protects. ⚠️ It is a notice ABOUT how the report behaves, not about which text carries authority, so it sits in neither the manifest nor the reviewed-constant disposition — it states a fact and decides nothing, and its target-language rendering is ordinary catalogue copy.',
    },
    {
